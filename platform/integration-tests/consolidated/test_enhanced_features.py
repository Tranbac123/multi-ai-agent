...
class TestConversationMemory:
    ...
    async def test_get_conversation_history(self):
        ...
        with patch.object(
            self.memory.redis_client, "lrange", return_value=mock_messages
        ):
    ...
    async def test_get_context(self):
        ...
        with patch.object(
            self.memory, "get_conversation_history", return_value=mock_history
        ):
...
class TestConversationAnalyzer:
    ...
    async def test_analyze_sentiment_positive(self):
        ...
        with patch.object(
            self.analyzer.memory, "get_conversation_history", return_value=mock_history
        ):
...
class TestAnalyticsService:
    ...
    async def test_track_message(self):
        ...
        with patch.object(self.analytics, "_increment_counter") as mock_increment:
            ...
            assert (
                mock_increment.call_count >= 3
            )  # messages_total, messages_user, messages_channel_web
    ...
    async def test_track_workflow_performance(self):
        ...
        with patch.object(self.analytics, "_increment_counter") as mock_increment:
            ...
            assert (
                mock_increment.call_count >= 2
            )  # workflow_FAQ_FLOW_total, workflow_FAQ_FLOW_success
    ...
    async def test_get_dashboard_metrics(self):
        ...
        with patch.object(
            self.analytics, "_get_message_metrics", return_value={"total": 100}
        ):
            with patch.object(self.analytics, "_get_workflow_metrics", return_value={}):
                with patch.object(
                    self.analytics, "_get_conversion_metrics", return_value={}
                ):
                    with patch.object(
                        self.analytics, "_get_performance_metrics", return_value={}
                    ):
                        with patch.object(
                            self.analytics, "_get_customer_metrics", return_value={}
                        ):
...
class TestWebSocketAPI:
    ...
    def test_websocket_message_flow(self):
        ...
        with client.websocket_connect("/ws/chat?session_id=test_session") as websocket:
            # Send message
            websocket.send_json({"type": "message", "text": "Hello, I need help"})
