...
class TestRealtimeIntegration:
    ...
    @pytest.mark.asyncio
    async def test_connection_registration_with_redis(
        self, connection_manager_with_redis
    ):
        ...
        assert (
            "session789"
            in connection_manager_with_redis.tenant_connections["tenant123"]
        )
    ...
    @pytest.mark.asyncio
    async def test_connection_unregistration_with_redis(
        self, connection_manager_with_redis
    ):
        ...
        assert (
            "session789"
            not in connection_manager_with_redis.tenant_connections["tenant123"]
        )
        assert (
            "session789"
            not in connection_manager_with_redis.user_connections["user456"]
        )
    ...
    async def test_broadcast_to_tenant_with_redis(self, connection_manager_with_redis):
        ...
        sent_count = await connection_manager_with_redis.broadcast_to_tenant(
            "tenant123", message
        )
    ...
    @pytest.mark.asyncio
    async def test_backpressure_handling_with_redis(
        self, backpressure_handler_with_redis
    ):
    ...
    @pytest.mark.asyncio
    async def test_backpressure_overflow_with_redis(
        self, backpressure_handler_with_redis
    ):
    ...
    @pytest.mark.asyncio
    async def test_backpressure_metrics_with_redis(
        self, backpressure_metrics_with_redis
    ):
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_realtime_flow(
        self, connection_manager_with_redis, backpressure_handler_with_redis
    ):
