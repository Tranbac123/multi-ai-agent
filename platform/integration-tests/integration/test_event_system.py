...
class TestEventSystem:
    ...
    async def test_event_producer(self):
        ...
        config = ProducerConfig(batch_size=5, batch_timeout=0.1, enable_batching=True)
        ...
        try:
            ...
            for i in range(10):
                await producer.publish(
                    EventType.AGENT_RUN_STARTED, {"run_id": f"run_{i}"}
                )
    ...
    async def test_event_consumer(self):
        ...
        config = ConsumerConfig(
            max_concurrent_events=3, event_timeout=1.0, enable_dlq=True
        )
    ...
    async def test_event_store(self):
        ...
        config = EventStoreConfig(max_events=1000, retention_days=1, batch_size=10)
        ...
        try:
            ...
            for i in range(5):
                metadata = EventMetadata(
                    event_type=EventType.AGENT_RUN_STARTED, tenant_id=f"tenant_{i % 2}"
                )
    ...
    async def test_integration_scenario(self):
        ...
        consumer_config = ConsumerConfig(max_concurrent_events=2, enable_dlq=True)
        ...
        try:
            ...
            for i in range(10):
                await producer.publish_agent_run_started(
                    run_id=f"run_{i}", tenant_id="tenant_123", agent_name="test_agent"
                )
    ...
    async def test_error_handling(self):
        ...
        try:
            ...
            await producer.publish(
                EventType.AGENT_RUN_STARTED, {"run_id": "failing_run"}
            )
