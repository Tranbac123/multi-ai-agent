    ...
    def test_autoscaling_configurations_have_consistent_namespaces(self):
        ...
        for doc in keda_docs:
            if "metadata" in doc and "namespace" in doc["metadata"]:
                ...
                ), f"KEDA resource {doc.get('metadata', {}).get('name', 'unknown')} should be in production namespace"
        ...
        for doc in hpa_docs:
            if "metadata" in doc and "namespace" in doc["metadata"]:
                ...
                assert (
                    namespace == "production"
                ), f"HPA resource {doc.get('metadata', {}).get('name', 'unknown')} should be in production namespace"
