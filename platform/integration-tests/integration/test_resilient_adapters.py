...
class MockToolAdapter(BaseToolAdapter):
    ...
    def __init__(
        self, name: str, config: AdapterConfig = None, should_fail: bool = False
    ):
...
class TestResilientAdapters:
    ...
    async def test_circuit_breaker(self):
        ...
        config = CircuitBreakerConfig(
            failure_threshold=3, recovery_timeout=1.0, success_threshold=2
        )
    ...
    async def test_retry_policy(self):
        ...
        config = RetryConfig(max_attempts=3, base_delay=0.1, strategy="exponential")
    ...
    async def test_health_checker(self):
        ...
        config = HealthCheckConfig(
            check_interval=0.1, failure_threshold=2, success_threshold=1
        )
    ...
    async def test_http_adapter(self):
        """Test HTTP adapter."""
        config = HTTPAdapterConfig(base_url="https://httpbin.org", timeout=10.0)
    ...
    async def test_database_adapter(self):
        ...
        config = DatabaseAdapterConfig(connection_string="sqlite:///test.db")
    ...
    async def test_integration_scenario(self):
        ...
        try:
            ...
            success_count = sum(
                1
                for r in results
                if isinstance(r, dict) and r.get("result") == "success"
            )
            assert success_count > 0
