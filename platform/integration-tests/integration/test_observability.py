...
class TestPrometheusMetrics:
    ...
    async def test_router_misroute_rate_metric(self):
        ...
        await metrics.record_router_decision(
            "tenant_001", "gpt-4", "gpt-3.5-turbo", True
        )
        await metrics.record_router_decision("tenant_001", "gpt-4", "gpt-4", False)
        await metrics.record_router_decision(
            "tenant_001", "gpt-3.5-turbo", "gpt-3.5-turbo", False
        )
    ...
    async def test_ws_backpressure_drops_metric(self):
        ...
        await metrics.record_ws_backpressure_drop(
            "tenant_001", "session_001", "slow_client"
        )
        await metrics.record_ws_backpressure_drop(
            "tenant_001", "session_002", "slow_client"
        )
    ...
    async def test_trace_hierarchy(self):
        ...
        with tracing.create_span("parent_operation") as parent_span:
            ...
            with tracing.create_span(
                "child_operation", parent=parent_span
            ) as child_span:
...
class TestSLOMonitoring:
    ...
    async def test_slo_availability_monitoring(self, redis_fixture):
        ...
        await slo_monitor.record_availability("api_gateway", True, time.time())
        await slo_monitor.record_availability(
            "api_gateway", False, time.time()
        )  # 1 failure out of 3
    ...
    async def test_slo_burn_rate_calculation(self, redis_fixture):
        ...
        for i in range(100):
            ...
            await slo_monitor.record_availability(
                "orchestrator", is_available, time.time()
            )

        # Check burn rate
        burn_rate = await slo_monitor.calculate_burn_rate(
    ...
    async def test_slo_alert_generation(self, redis_fixture):
        ...
        for i in range(50):
            ...
            await slo_monitor.record_availability(
                "billing_service", is_available, time.time()
            )
        ...
        assert len(alerts) > 0
        billing_alerts = [
            alert for alert in alerts if alert["service"] == "billing_service"
        ]
...
class TestGrafanaDashboards:
            ...
            panels=[
                {
                    ...
                    "type": "graph",
                    "targets": [
                        {"expr": "agent_run_latency_ms", "legendFormat": "Latency (ms)"}
                    ],
                },
                {
    ...
    async def test_dashboard_validation(self):
        ...
        valid_dashboard = {
            ...
            "panels": [{"title": "Test Panel", "type": "graph", "targets": []}],
        }
...
class TestObservabilityIntegration:
    ...
    async def test_end_to_end_observability(self, redis_fixture):
        ...
        with tracing.create_span("api_request") as span:
            ...
            await metrics.record_tool_execution(
                "tenant_001", "order_lookup", True, 500.0
            )
    ...
    async def _simulate_request(self, tracing, metrics, request_id):
        ...
        with tracing.create_span(f"request_{request_id}") as span:
            ...
            # Record metrics
            await metrics.record_agent_run_latency(
                "tenant_001", "workflow_001", 1000.0 + request_id
            )
