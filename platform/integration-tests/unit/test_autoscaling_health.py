...
from infra.k8s.autoscaling.keda_scalers import (
    KEDAScaler,
    KEDAManager,
    ScalerConfig,
    ScalerType,
...
class TestKEDAScaler:
    ...
    async def test_nats_queue_scaler(self, mock_redis):
        ...
        # Mock queue depth
        mock_redis.get = AsyncMock(return_value=b"15.0")
    ...
    async def test_memory_scaler(self, mock_redis):
        ...
        scaler = KEDAScaler(config, mock_redis)
        scaler.metrics.replicas = 3  # Set current replicas above minimum

        # Mock memory usage
        mock_redis.get = AsyncMock(return_value=b"0.2")
...
class TestKEDAManager:
    ...
    async def test_update_scaler_config(self, mock_redis):
        ...
        success = await manager.update_scaler_config(
            "orchestrator-nats", {"max_replicas": 25, "scale_up_threshold": 15.0}
        )
