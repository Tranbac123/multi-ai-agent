...
from apps.router_service.core.early_exit_escalation import EarlyExitEscalation, EscalationReason
...
class TestFeatureExtractorHardening:
    ...
    async def test_feature_extraction_caching(self, feature_extractor):
        ...
        features1 = await feature_extractor.extract_features(request, tenant_id, user_id)
    ...
    async def test_deterministic_feature_extraction(self, feature_extractor):
        ...
        features1 = await feature_extractor.extract_features(request, tenant_id, user_id)
        features2 = await feature_extractor.extract_features(request, tenant_id, user_id)
...
class TestCalibratedClassifierHardening:
    ...
    async def test_deterministic_fallback(self, classifier):
        ...
        results = []
        for _ in range(5):
            tier, confidence, should_escalate = await classifier._deterministic_fallback(features)
...
class TestEarlyExitEscalationHardening:
    ...
    async def test_tenant_json_requirements(self, early_exit):
        ...
        assert early_exit._meets_tenant_requirements(features_good, requirements) is True
        assert early_exit._meets_tenant_requirements(features_bad, requirements) is False
...
class TestRouterV2Performance:
    ...
    async def test_high_concurrency_routing(self, router_v2):
        ...
        num_requests = 100
        requests = [
            ({"message": f"Concurrent request {i}", "user_id": f"user{i}"}, f"tenant{i % 10}", f"user{i}")
        ...
        for decision in decisions:
            ...
            assert decision.decision_time_ms < 200  # More realistic threshold for high concurrency
