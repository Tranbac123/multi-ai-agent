...
class TestSLOMonitor:
    ...
    async def test_check_slo_status_warning(self, mock_redis):
        ...
        alert = await monitor.check_slo_status("orchestrator", "workflow_completion_rate")
    ...
    async def test_check_all_slos(self, mock_redis):
        ...
        for alert in alerts:
            ...
            assert alert.status in [SLOStatus.HEALTHY, SLOStatus.WARNING, SLOStatus.CRITICAL, SLOStatus.UNKNOWN]
    ...
    async def test_calculate_burn_rate_availability(self, mock_redis):
        ...
        burn_rate = await monitor._calculate_burn_rate("test-service", "availability", target)
    ...
    async def test_calculate_burn_rate_latency(self, mock_redis):
        ...
        burn_rate = await monitor._calculate_burn_rate("test-service", "p50_latency", target)
        assert burn_rate < 0.1  # Should be healthy
    ...
    async def test_calculate_burn_rate_error_rate(self, mock_redis):
        ...
        burn_rate = await monitor._calculate_burn_rate("test-service", "misroute_rate", target)
        assert burn_rate < 0.1  # Should be healthy
    ...
    async def test_get_current_metric_value(self, mock_redis):
        ...
        value = await monitor._get_current_metric_value("unknown-service", "availability")
        assert value == 0.0
...
class TestIntegration:
    ...
    async def test_slo_monitoring_workflow(self, mock_redis):
        ...
        total_summary = (summary['healthy'] + summary['warning'] + 
                        summary['critical'] + summary['unknown'])
