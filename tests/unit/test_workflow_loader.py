...
from libs.workflow_loader import WorkflowLoader, WorkflowValidationError
...
class TestWorkflowLoader:
    ...
    def test_workflow_extends(self):
        ...
        assert len(workflow.nodes) == 3  # start + process + end
        assert len(workflow.edges) == 2  # start->process + process->end
    ...
    def test_workflow_insert_after(self):
        ...
        workflow_data = {
            ...
            "insert_after": {
                "process": [
                    {"id": "validation", "type": "validator", "config": {"rules": ["required"]}}
                ]
            }
        }
        ...
        assert any(node["id"] == "validation" for node in workflow.nodes)
    ...
    def test_workflow_patch(self):
        ...
        process_node = next(node for node in workflow.nodes if node["id"] == "process")
        assert process_node["config"]["model"] == "gpt-4-turbo"
        assert process_node["config"]["temperature"] == 0.7
    ...
    def test_workflow_budget_validation(self):
        ...
        assert workflow.budget.max_cost == 1.0
        assert workflow.budget.max_tokens == 1000
        assert workflow.budget.max_duration == 30
    ...
    def test_workflow_validation_gates(self):
        ...
        assert len(workflow.validation_gates) == 2
        assert workflow.validation_gates[0]["name"] == "input_validation"
        assert workflow.validation_gates[1]["name"] == "output_validation"
    ...
    def test_workflow_validation_error_missing_name(self):
        ...
        with pytest.raises(WorkflowValidationError, match="Missing required field: name"):
    ...
    def test_workflow_validation_error_invalid_node_type(self):
        ...
        workflow_data = {
            ...
            "version": "1.0.0",
            "nodes": [
                {"id": "start", "type": "invalid_type", "config": {}}
            ],
            "edges": []
        }
        ...
        with pytest.raises(WorkflowValidationError, match="Invalid node type: invalid_type"):
            loader.load_workflow(workflow_data)
    ...
    def test_workflow_validation_error_orphaned_edges(self):
        ...
        with pytest.raises(WorkflowValidationError, match="Edge references nonexistent node: nonexistent"):
            loader.load_workflow(workflow_data)
    ...
    def test_workflow_validate_budget(self):
        ...
        # Valid budget
        assert loader._validate_budget(workflow, {"cost": 0.5, "tokens": 500, "duration": 15}) is True
        
        # Exceeded cost
        assert loader._validate_budget(workflow, {"cost": 1.5, "tokens": 500, "duration": 15}) is False
        
        # Exceeded tokens
        assert loader._validate_budget(workflow, {"cost": 0.5, "tokens": 1500, "duration": 15}) is False
        
        # Exceeded duration
        assert loader._validate_budget(workflow, {"cost": 0.5, "tokens": 500, "duration": 45}) is False
...
class TestWorkflowLoaderPropertyBased:
    ...
    def test_workflow_node_ids_unique(self, node_ids):
        ...
        workflow_data = {
            "name": "test_workflow",
            "version": "1.0.0",
            "nodes": [{"id": node_id, "type": "start", "config": {}} for node_id in node_ids],
            "edges": []
        }
        ...
        if len(set(node_ids)) == len(node_ids):  # All unique
            workflow = loader.load_workflow(workflow_data)
            assert len(workflow.nodes) == len(node_ids)
        else:
