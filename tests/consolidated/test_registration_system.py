    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            response = await client.post(
                f"{BASE_URL}/api/register", json=registration_data
            )
            if response.status_code == 200:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   - Messages: {usage['messages_used_this_month']}/{usage['messages_limit']} ({usage['messages_usage_percent']:.1f}%)"
                )
                print(
                    f"   - Customers: {usage['customers_created']}/{usage['customers_limit']} ({usage['customers_usage_percent']:.1f}%)"
                )
                print(
                    f"   - Storage: {usage['storage_used_gb']:.1f}GB/{usage['storage_limit_gb']}GB ({usage['storage_usage_percent']:.1f}%)"
                )
        ...
        try:
            upgrade_data = {"package_type": "plus", "billing_cycle": "monthly"}
            response = await client.post(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   - Features: {package['has_analytics']}, {package['has_priority_support']}"
                )
            else:
