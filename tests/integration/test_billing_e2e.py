...
from apps.api_gateway.middleware.quota_enforcement import QuotaEnforcementMiddleware, QuotaStatus as MiddlewareQuotaStatus
...
class TestBillingE2E:
    ...
    async def services(self, redis_mock):
        ...
        webhook_aggregator = WebhookAggregator(redis_mock, usage_tracker, billing_engine)
        invoice_preview_service = InvoicePreviewService(redis_mock, usage_tracker, billing_engine)
    ...
    async def test_usage_recording_e2e(self, services):
        ...
        current_usage = await services["usage_tracker"].get_current_usage(tenant_id, UsageType.API_CALLS)
    ...
    async def test_invoice_preview_e2e(self, services):
        ...
        preview = await services["invoice_preview_service"].generate_invoice_preview(tenant_id)
    ...
    async def test_webhook_aggregation_e2e(self, services):
        ...
        api_usage = await services["usage_tracker"].get_current_usage(tenant_id, UsageType.API_CALLS)
        tool_usage = await services["usage_tracker"].get_current_usage(tenant_id, UsageType.TOOL_CALLS)
    ...
    async def test_quota_middleware_e2e(self, redis_mock):
        ...
        check = await middleware._check_single_quota(tenant_id, usage_type, requested_amount)
    ...
    async def test_invoice_preview_caching(self, services):
        ...
        # Generate preview
        preview1 = await services["invoice_preview_service"].generate_invoice_preview(tenant_id)
    ...
    async def test_quota_status_aggregation(self, services):
        ...
        api_quota = next((q for q in quota_limits if q.usage_type == UsageType.API_CALLS), None)
    ...
    async def test_billing_cycle_integration(self, services):
        ...
        assert abs(total_calculated - preview.subtotal) < 0.01  # Allow for floating point precision
    ...
    async def test_quota_reset_simulation(self, services):
        ...
        await services["invoice_preview_service"].redis_client.set(
            f"quota:{tenant_id}:{UsageType.API_CALLS.value}",
            "10"  # Very low limit
        )
        ...
        await services["usage_tracker"].redis_client.delete(f"usage:{tenant_id}:{UsageType.API_CALLS.value}")
