...
class TestResourceConfigurationValidation:
    ...
    def test_resource_limits_are_valid_cpu_format(self):
        ...
        for deployment in deployments:
            ...
            assert cpu_limit.endswith("m") or cpu_limit.endswith("n") or cpu_limit.isdigit()
            
            # Check CPU requests format
            cpu_request = resources["requests"]["cpu"]
            assert isinstance(cpu_request, str)
            assert cpu_request.endswith("m") or cpu_request.endswith("n") or cpu_request.isdigit()
    ...
    def test_resource_limits_are_valid_memory_format(self):
        ...
        for deployment in deployments:
            ...
            assert memory_limit.endswith("Gi") or memory_limit.endswith("Mi") or memory_limit.endswith("Ki")
            
            # Check memory requests format
            memory_request = resources["requests"]["memory"]
            assert isinstance(memory_request, str)
            assert memory_request.endswith("Gi") or memory_request.endswith("Mi") or memory_request.endswith("Ki")
    ...
    def test_resource_limits_are_consistent(self):
        ...
        for deployment in deployments:
            ...
            assert cpu_limit_millicores >= cpu_request_millicores, f"CPU limit should be >= CPU request for {deployment['metadata']['name']}"
            ...
            assert memory_limit_mib >= memory_request_mib, f"Memory limit should be >= Memory request for {deployment['metadata']['name']}"
    ...
    def test_resource_limits_have_consistent_units(self):
        ...
        for deployment in deployments:
            ...
            if cpu_limit.endswith("m"):
                assert cpu_request.endswith("m"), f"CPU request should use same unit as limit for {deployment['metadata']['name']}"
            elif cpu_limit.endswith("n"):
                assert cpu_request.endswith("n"), f"CPU request should use same unit as limit for {deployment['metadata']['name']}"
            elif cpu_limit.isdigit():
                assert cpu_request.isdigit(), f"CPU request should use same unit as limit for {deployment['metadata']['name']}"
            ...
            # Both should use the same unit type
            if memory_limit.endswith("Gi"):
                assert memory_request.endswith("Gi"), f"Memory request should use same unit as limit for {deployment['metadata']['name']}"
            elif memory_limit.endswith("Mi"):
                assert memory_request.endswith("Mi"), f"Memory request should use same unit as limit for {deployment['metadata']['name']}"
            elif memory_limit.endswith("Ki"):
                assert memory_request.endswith("Ki"), f"Memory request should use same unit as limit for {deployment['metadata']['name']}"
