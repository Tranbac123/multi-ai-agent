"""Integration tests for autoscaling and security configuration validation integration."""
...
class TestAutoscalingSecurityValidationIntegration:
    """Test autoscaling and security configuration validation integration."""
    ...
    def test_all_configurations_are_valid_yaml(self):
        ...
        for file_path in files:
            ...
            assert len(docs) > 0, f"File {file_path} should contain at least one document"
            
            # Check that all documents are valid
            for doc in docs:
                assert isinstance(doc, dict), f"Each document in {file_path} should be a dictionary"
                assert "apiVersion" in doc, f"Each document in {file_path} should have apiVersion"
                assert "kind" in doc, f"Each document in {file_path} should have kind"
                assert "metadata" in doc, f"Each document in {file_path} should have metadata"
    ...
    def test_all_configurations_have_valid_kinds(self):
        ...
        valid_kinds = [
            ...
            "ScaledObject", "HorizontalPodAutoscaler", "ConfigMap", "Secret", "NetworkPolicy", "Namespace"
        ]
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "namespace" in doc["metadata"]:
                    ...
                    assert namespace == "production", f"Resource {doc.get('metadata', {}).get('name', 'unknown')} in {file_path} should be in production namespace"
            ...
            for doc in docs:
                if "metadata" in doc:
                    ...
                    if "namespace" in metadata:
                        ...
                        assert len(metadata["namespace"]) <= 63  # Kubernetes namespace length limit
