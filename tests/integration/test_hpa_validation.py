...
class TestHPAValidation:
    ...
    def test_hpa_targets_existing_deployments(self):
        ...
        for hpa in hpas:
            ...
            assert target_name in deployment_names, f"HPA {hpa['metadata']['name']} targets non-existent deployment {target_name}"
    ...
    def test_hpa_has_proper_namespace(self):
        ...
        for hpa in hpas:
            ...
            assert namespace == "production", f"HPA {hpa['metadata']['name']} should be in production namespace"
    ...
    def test_hpa_has_resource_metrics(self):
        ...
        for hpa in hpas:
            ...
            assert len(resource_metrics) > 0, f"HPA {hpa['metadata']['name']} should have resource metrics"
            
            # Check CPU and memory metrics
            cpu_metric = next((m for m in resource_metrics if m["resource"]["name"] == "cpu"), None)
            memory_metric = next((m for m in resource_metrics if m["resource"]["name"] == "memory"), None)
            
            assert cpu_metric is not None, f"HPA {hpa['metadata']['name']} should have CPU metric"
            assert memory_metric is not None, f"HPA {hpa['metadata']['name']} should have memory metric"
    ...
    def test_hpa_has_behavior_configuration(self):
        ...
        for hpa in hpas:
            assert "behavior" in hpa["spec"], f"HPA {hpa['metadata']['name']} should have behavior configuration"
    ...
    def test_hpa_has_reasonable_scaling_limits(self):
        ...
        for hpa in hpas:
            ...
            assert min_replicas >= 1, f"HPA {hpa['metadata']['name']} minReplicas should be at least 1"
            assert max_replicas <= 50, f"HPA {hpa['metadata']['name']} maxReplicas should be at most 50"
            assert max_replicas >= min_replicas, f"HPA {hpa['metadata']['name']} maxReplicas should be >= minReplicas"
            
            # Check scaling ratio is reasonable (not too aggressive)
            scaling_ratio = max_replicas / min_replicas
            assert scaling_ratio <= 10, f"HPA {hpa['metadata']['name']} scaling ratio should be at most 10x"
            ...
            for metric in metrics:
                if metric.get("type") == "Resource":
                    ...
                    if "averageUtilization" in target:
                        ...
                        assert utilization > 0, f"HPA {hpa['metadata']['name']} metric {resource['name']} utilization should be > 0"
                        assert utilization <= 100, f"HPA {hpa['metadata']['name']} metric {resource['name']} utilization should be <= 100"
                    
                    # Check target value is reasonable
                    if "averageValue" in target:
                        value = target["averageValue"]
                        assert value > 0, f"HPA {hpa['metadata']['name']} metric {resource['name']} value should be > 0"
        ...
        for hpa in hpas:
            ...
            for metric in metrics:
                if metric.get("type") == "Resource":
                    ...
                    assert resource["name"] in ["cpu", "memory"], f"HPA {hpa['metadata']['name']} has invalid resource name: {resource['name']}"
                    
                    # Check target is properly configured
                    target = resource["target"]
                    assert "averageUtilization" in target or "averageValue" in target, f"HPA {hpa['metadata']['name']} metric {resource['name']} should have target"
