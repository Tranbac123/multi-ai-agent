...
class TestWebSocketBackpressure:
    ...
    async def test_slow_client_drop_intermediates(self):
        ...
        messages = [
            {"type": "intermediate", "content": f"Message {i}"}
            for i in range(10)
        ]
    ...
    async def test_deliver_final_message(self):
        ...
        intermediate_messages = [
            {"type": "intermediate", "content": f"Step {i}"}
            for i in range(5)
        ]
        ...
        final_calls = [call for call in slow_client.send.call_args_list 
    ...
    async def test_priority_message_handling(self):
        ...
        priority_calls = [call for call in slow_client.send.call_args_list 
    ...
    async def test_message_ordering(self):
        ...
        sequences = [msg.get("sequence") for msg in delivered_messages if "sequence" in msg]
