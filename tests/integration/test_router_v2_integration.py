...
class TestRouterV2Integration:
    ...
    async def test_router_decision_latency_p50_under_50ms(self, router_v2_with_redis):
        ...
        for i in range(100):
            ...
            decision = await router_v2_with_redis.route_request(
                request, "tenant123", f"user{i}"
            )
    ...
    async def test_canary_gating_by_tenant(self, router_v2_with_redis):
        ...
        assert (
            0.05 <= canary_percentage <= 0.10
        ), f"Canary percentage {canary_percentage} not in 5-10% range"
    ...
    async def test_expected_vs_actual_cost_within_tolerance(self, router_v2_with_redis):
        ...
        for i in range(50):
            ...
            actual_cost = (
                decision.tier.value * 0.1 + (i % 3) * 0.05
            )  # Add some variation
            await router_v2_with_redis.record_outcome(
    ...
    async def test_early_exit_acceptance_for_strict_json(self, router_v2_with_redis):
        ...
        for i, message in enumerate(early_exit_requests):
            ...
            if (
                ...
                and decision.escalation_decision.early_exit_tier
                and decision.escalation_decision.early_exit_tier == Tier.A
            ):
                early_exit_count += 1
    ...
    async def test_escalation_for_complex_requests(self, router_v2_with_redis):
        ...
        decision = await router_v2_with_redis.route_request(
            complex_request, "tenant123", "user123"
        )
        ...
        assert decision.tier in [
            Tier.B,
            Tier.C,
        ], f"Complex request not escalated, tier: {decision.tier}"
    ...
    async def test_misroute_rate_calculation(self, router_v2_with_redis):
        ...
        assert (
            0 <= misroute_rate <= 1
        ), f"Misroute rate {misroute_rate} not in valid range"
    ...
    async def test_tenant_isolation(self, router_v2_with_redis):
        ...
        for i in range(10):
            ...
            decision1 = await router_v2_with_redis.route_request(
                request1, "tenant1", f"user{i}"
            )
            ...
            decision2 = await router_v2_with_redis.route_request(
                request2, "tenant2", f"user{i}"
            )
