...
class TestSagaOrchestration:
                ...
                with patch.object(crm_adapter, "create_lead") as mock_crm:
                    ...
                    result = await saga.execute_saga(
                        [
                            ...
                            (
                                payment_adapter.process_payment,
                                payment_adapter.compensate,
                            ),
        ...
        with patch.object(email_adapter, "send_email") as mock_email:
            with patch.object(payment_adapter, "process_payment") as mock_payment:
                with patch.object(crm_adapter, "create_lead") as mock_crm:
                    with patch.object(
                        payment_adapter, "compensate"
                    ) as mock_payment_comp:
                        with patch.object(
                            email_adapter, "compensate"
                        ) as mock_email_comp:
                            mock_email.return_value = {
                                "success": True,
                                "message_id": "email_001",
                            ...
                            mock_payment_comp.return_value = {
                                "success": True,
                                "refund_id": "refund_001",
                            }
                            mock_email_comp.return_value = {
                                "success": True,
                                "cancelled": True,
                            }
    ...
    async def test_saga_timeout_handling(self):
        ...
        result = await saga.execute_saga([(slow_operation, compensation)])
