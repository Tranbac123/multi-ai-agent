...
class TestQuotaEnforcer:
    ...
    async def test_usage_increment(self, quota_enforcer, mock_db_client):
        ...
        mock_db_client.get_session.return_value.__aenter__.return_value.execute.return_value = (
            None
        )
        mock_db_client.get_session.return_value.__aenter__.return_value.commit.return_value = (
            None
        )

        result = await quota_enforcer.increment_usage("test-tenant", "api_calls", 1)
...
class TestQuotaEnforcementMiddleware:
    ...
    async def test_quota_enforcement_decorator_allows_request(
        self, middleware, mock_quota_enforcer
    ):
        """Test quota enforcement decorator allows request when within limits."""
        # Mock quota check to allow request
        mock_quota_enforcer.check_quota.return_value = (
            True,
            {"remaining": {"api_calls": 100}},
        )
    ...
    async def test_quota_enforcement_decorator_denies_request(
        self, middleware, mock_quota_enforcer
    ):
    ...
    async def test_rate_limit_enforcement_decorator(
        self, middleware, mock_quota_enforcer
    ):
