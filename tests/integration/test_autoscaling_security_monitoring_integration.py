"""Integration tests for autoscaling and security configuration monitoring integration."""
...
class TestAutoscalingSecurityMonitoringIntegration:
    """Test autoscaling and security configuration monitoring integration."""
    
    def test_keda_monitoring_integration(self):
        """Test KEDA monitoring integration."""
    ...
    def test_hpa_monitoring_integration(self):
        """Test HPA monitoring integration."""
    ...
    def test_health_probes_monitoring_integration(self):
        """Test health probes monitoring integration."""
    ...
    def test_networkpolicy_monitoring_integration(self):
        """Test NetworkPolicy monitoring integration."""
        ...
        assert len(network_policies) > 0, "Network policy configuration should have NetworkPolicies"
    ...
    def test_all_configurations_have_consistent_monitoring_versions(self):
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "labels" in doc["metadata"] and "app.kubernetes.io/version" in doc["metadata"]["labels"]:
                    version = doc["metadata"]["labels"]["app.kubernetes.io/version"]
                    versions.add(version)
        
        # All resources should use the same version
        assert len(versions) <= 1, f"All resources should use the same version, but found: {versions}"
    ...
    def test_all_configurations_have_consistent_monitoring_managed_by(self):
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "labels" in doc["metadata"] and "app.kubernetes.io/managed-by" in doc["metadata"]["labels"]:
        ...
        assert len(managed_by_values) <= 1, f"All resources should use the same managed-by, but found: {managed_by_values}"
    ...
    def test_all_configurations_have_consistent_monitoring_part_of(self):
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "labels" in doc["metadata"] and "app.kubernetes.io/part-of" in doc["metadata"]["labels"]:
                    part_of = doc["metadata"]["labels"]["app.kubernetes.io/part-of"]
                    part_of_values.add(part_of)
        
        # All resources should use the same part-of
        assert len(part_of_values) <= 1, f"All resources should use the same part-of, but found: {part_of_values}"
    ...
    def test_all_configurations_have_consistent_monitoring_namespaces(self):
        ...
        assert len(namespaces) <= 1, f"All resources should use the same namespace, but found: {namespaces}"
        assert "production" in namespaces, "All resources should be in production namespace"
    ...
    def test_all_configurations_have_consistent_monitoring_created_by(self):
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "annotations" in doc["metadata"] and "app.kubernetes.io/created-by" in doc["metadata"]["annotations"]:
                    created_by = doc["metadata"]["annotations"]["app.kubernetes.io/created-by"]
                    created_by_values.add(created_by)
        
        # All resources should use the same created-by
        assert len(created_by_values) <= 1, f"All resources should use the same created-by, but found: {created_by_values}"
    ...
    def test_all_configurations_have_consistent_monitoring_descriptions(self):
        ...
        for file_path in files:
            ...
            for doc in docs:
                if "metadata" in doc and "annotations" in doc["metadata"] and "description" in doc["metadata"]["annotations"]:
