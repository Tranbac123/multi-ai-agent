...
from eval.golden_tasks.comprehensive_tasks import (
    get_all_tasks, TaskCategory, TaskDifficulty, ExpectedTier
)
...
class TestEvaluationRunner:
    ...
    async def test_evaluation_run_failure(self, runner, evaluation_config):
        ...
        runner.llm_judge.evaluate_response = AsyncMock(side_effect=Exception("API Error"))
...
class TestEvaluationIntegration:
    ...
    async def test_golden_tasks_integration(self, temp_dir):
        ...
        easy_tasks = [task for task in faq_tasks if task.metadata.get("difficulty") == "easy"]
        
        assert len(easy_tasks) > 0
        assert all(task.metadata.get("difficulty") == "easy" for task in easy_tasks)
        
        # Test getting tasks by tier
        slm_a_tasks = [task for task in faq_tasks if task.metadata.get("expected_tier") == "SLM_A"]
        
        assert len(slm_a_tasks) > 0
        assert all(task.metadata.get("expected_tier") == "SLM_A" for task in slm_a_tasks)
    ...
    async def test_episode_replay_integration(self, temp_dir):
        ...
        with patch('redis.asyncio.from_url') as mock_redis:
            ...
            replay_engine = EpisodeReplayEngine(
                ...
                storage_path=str(temp_dir / "episodes")
            )
    ...
    async def test_metrics_integration(self, temp_dir):
        ...
        success_rate = metrics_data["successful_tasks"] / metrics_data["completed_tasks"]
