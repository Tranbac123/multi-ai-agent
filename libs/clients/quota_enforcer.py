...
class QuotaEnforcer:
    ...
    async def check_quota(
        self, tenant_id: UUID, quota_type: str, current_usage: int, db: AsyncSession
    ) -> Dict[str, Any]:
    ...
    async def get_current_usage(
        self, tenant_id: UUID, quota_type: str, db: AsyncSession
    ) -> int:
    ...
    async def _get_message_usage(self, tenant_id: UUID, db: AsyncSession) -> int:
        ...
        try:
            ...
            logger.error(
                "Failed to get message usage", tenant_id=tenant_id, error=str(e)
            )
            return 0
    ...
    async def _get_customer_count(self, tenant_id: UUID, db: AsyncSession) -> int:
        ...
        try:
            ...
            logger.error(
                "Failed to get customer count", tenant_id=tenant_id, error=str(e)
            )
            return 0
    ...
    async def _get_agent_count(self, tenant_id: UUID, db: AsyncSession) -> int:
        ...
        try:
            stmt = select(func.count("users.id")).where(
                and_(
                    "users.tenant_id == tenant_id", "users.role.in_(['agent', 'admin'])"
                )
            )
            result = await db.execute(stmt)
            count = result.scalar() or 0
    ...
    async def _get_storage_usage(self, tenant_id: UUID, db: AsyncSession) -> int:
        ...
        try:
            ...
            logger.error(
                "Failed to get storage usage", tenant_id=tenant_id, error=str(e)
            )
            return 0
    ...
    async def enforce_quota(
        ...
        try:
            ...
            quota_check = await self.check_quota(
                tenant_id, quota_type, current_usage, db
            )
    ...
    async def get_quota_status(
        ...
        try:
            ...
            logger.error(
                "Failed to get quota status", tenant_id=tenant_id, error=str(e)
            )
            return {}
