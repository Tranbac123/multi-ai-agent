...
class QuotaEnforcer:
    ...
    async def check_quota(
        self, tenant_id: str, plan: str, scope: str, usage_type: str
        ...
        try:
            async with self.db_client.get_session() as session:
                ...
                result = await session.execute(
                    text("SELECT quotas FROM plans WHERE id = :plan"), {"plan": plan}
                )
                row = result.fetchone()
                if row:
                    return row[0]
    ...
    async def _get_current_usage(self, tenant_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error(
                "Failed to get current usage", error=str(e), tenant_id=tenant_id
            )
            return {}

    async def increment_usage(
        self, tenant_id: str, usage_type: str, amount: int
    ) -> bool:
