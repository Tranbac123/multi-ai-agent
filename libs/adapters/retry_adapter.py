...
class RetryAdapter:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                ...
                if not self._is_retryable(e):
                    logger.warning(
                        "Non-retryable exception caught",
                        exception=str(e),
                        function=func.__name__,
                    )
                    break
    ...
    def get_stats(self) -> dict:
        ...
        if stats["total_calls"] > 0:
            ...
            stats["avg_retry_delay"] = stats["total_retry_delay"] / max(
...
QUICK_RETRY = RetryConfig(
    max_attempts=2, base_delay=0.1, max_delay=1.0, exponential_base=2.0, jitter=True
)
...
LONG_RETRY = RetryConfig(
    max_attempts=5, base_delay=2.0, max_delay=120.0, exponential_base=2.0, jitter=True
)
