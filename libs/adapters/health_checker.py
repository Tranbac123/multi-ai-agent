    ...
    async def _perform_check(self):
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.check_function(), timeout=self.config.timeout
            )
            ...
            if is_healthy:
                ...
                await self._on_failure(
                    f"Health check returned unhealthy result: {result}"
                )
        ...
        async with self._lock:
            ...
            if self.consecutive_failures >= self.config.failure_threshold:
                if self.status != HealthStatus.UNHEALTHY:
                    ...
                    logger.warning(
                        f"Health checker {self.name} status changed to UNHEALTHY: {reason}"
                    )
            elif self.status == HealthStatus.UNKNOWN:
                self.status = HealthStatus.DEGRADED
                logger.warning(
                    f"Health checker {self.name} status changed to DEGRADED: {reason}"
                )
