...
class DLQHandler:
    ...
    async def _process_retries(self):
        ...
        async with self._lock:
            for i, dlq_entry in enumerate(self.dlq_events):
                ...
                if now >= next_retry and dlq_entry["retry_count"] < self.config.max_retry_attempts:
            ...
            if success:
                ...
                async with self._lock:
                    if index < len(self.dlq_events):
                        ...
                        delay = self.config.retry_delay * (self.config.retry_backoff_multiplier ** dlq_entry["retry_count"])
                        dlq_entry["next_retry"] = (datetime.utcnow() + timedelta(seconds=delay)).isoformat()
                
                self._stats["events_retried"] += 1
                logger.warning(f"Event {event.metadata.event_id} retry failed, will retry later")
    ...
    async def _cleanup_expired_events(self):
        ...
        async with self._lock:
            ...
            self.dlq_events = [
                entry for entry in self.dlq_events
