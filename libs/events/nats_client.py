...
class NATSClient:
    ...
    async def _create_streams(self):
        ...
        for stream_config in streams:
            try:
                ...
                logger.warning(
                    "Stream creation failed", name=stream_config["name"], error=str(e)
                )
    ...
    async def publish_event(
        ...
        try:
            ...
            ack = await self.js.publish(
                subject, json.dumps(event_payload).encode(), headers=event_headers
            )
    ...
    async def subscribe_to_events(
        ...
        try:
            ...
            consumer = await self.js.add_consumer(
                stream_name=f"{event_type}_events", config=consumer_config
            )
            ...
            asyncio.create_task(
                self._process_messages(subscription, handler, event_type)
            )
    ...
    async def _process_messages(
        self, subscription: nats.Subscription, handler: Callable, event_type: str
    ):
        ...
        while True:
            try:
                ...
                logger.error(
                    "Message processing error", event_type=event_type, error=str(e)
                )
                await asyncio.sleep(1)  # Wait before retrying

    async def _send_to_dlq(self, event_type: str, message: nats.Msg, error: str):
        ...
        try:
            ...
            logger.error(
                "Failed to send message to DLQ", event_type=event_type, error=str(e)
            )
    ...
    async def subscribe_to_dlq(self, event_type: str, handler: Callable):
        ...
        try:
            ...
            subscription = await self.js.subscribe(
                dlq_subject, handler=self._handle_dlq_message, queue="dlq_processor"
            )

            self.dlq_handlers[event_type] = handler

            logger.info(
                "Subscribed to DLQ", event_type=event_type, dlq_subject=dlq_subject
            )
    ...
    async def get_stream_info(self, stream_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "Failed to get stream info", stream_name=stream_name, error=str(e)
            )
            return None
