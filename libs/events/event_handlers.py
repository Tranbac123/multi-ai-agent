...
from .event_types import (
    EventType, AgentRunEvent, ToolCallEvent, IngestDocEvent,
    UsageMeteredEvent, RouterDecisionEvent, WebSocketMessageEvent,
    BillingEvent, AuditLogEvent
)
...
class AgentRunEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("Failed to handle agent run event", 
                        error=str(e), 
    ...
    async def _update_metrics(self, event: AgentRunEvent) -> None:
        ...
        from observability.metrics.prometheus_metrics import (
            agent_runs_total, agent_run_duration_seconds, agent_tokens_used
        )
...
class ToolCallEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("Failed to handle tool call event", 
                        error=str(e), 
                        event_data=event_data)
            raise
...
class IngestDocEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info("Document ingestion event", 
                       doc_id=event.doc_id,
            ...
            logger.error("Failed to handle document ingestion event", 
                        error=str(e), 
                        event_data=event_data)
            raise
    ...
    async def _update_metrics(self, event: IngestDocEvent) -> None:
        ...
        from observability.metrics.prometheus_metrics import (
            documents_ingested_total, document_ingestion_duration_seconds,
...
class UsageMeteredEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Log usage metering
            logger.info("Usage metering event", 
            ...
            logger.error("Failed to handle usage metering event", 
                        error=str(e), 
                        event_data=event_data)
            raise
    ...
    async def _update_metrics(self, event: UsageMeteredEvent) -> None:
        ...
        from observability.metrics.prometheus_metrics import (
            usage_metered_total, usage_cost_usd_total
        )
...
class RouterDecisionEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Log router decision
            logger.info("Router decision event", 
            ...
            logger.error("Failed to handle router decision event", 
                        error=str(e), 
                        event_data=event_data)
            raise
    ...
    async def _update_metrics(self, event: RouterDecisionEvent) -> None:
        ...
        from observability.metrics.prometheus_metrics import (
            router_decisions_total, router_confidence_histogram
        )
...
class WebSocketMessageEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info("WebSocket message event", 
            ...
            logger.error("Failed to handle WebSocket message event", 
                        error=str(e), 
                        event_data=event_data)
            raise
...
class BillingEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("Failed to handle billing event", 
                        error=str(e), 
                        event_data=event_data)
            raise
    ...
    async def _update_metrics(self, event: BillingEvent) -> None:
        ...
        from observability.metrics.prometheus_metrics import (
            billing_events_total, billing_amount_usd_total
        )
...
class AuditLogEventHandler(EventHandler):
    ...
    async def handle(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("Failed to handle audit log event", 
                        error=str(e), 
                        event_data=event_data)
            raise
