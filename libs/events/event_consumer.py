...
class EventConsumer:
    ...
    def __init__(self, event_bus: EventBus, config: ConsumerConfig = None):
        ...
        self.dlq_handler = DLQHandler(self.config.dlq_config) if self.config.enable_dlq else None
        self._stats = {
            "events_processed": 0,
            "events_failed": 0,
            "events_retried": 0,
    ...
    async def subscribe_to_event_type(self, event_type: EventType, queue_group: Optional[str] = None):
    ...
    async def subscribe_to_subject(self, subject: str, queue_group: Optional[str] = None):
    ...
    async def _process_event(self, event: Event):
        ...
        if not handler:
            logger.warning(f"No handler for event type {event.metadata.event_type.value}")
            return
    ...
    async def _send_to_dlq(self, event: Event, reason: str):
        ...
        if self.dlq_handler:
            ...
            logger.error(f"Event {event.metadata.event_id} failed and no DLQ configured: {reason}")
