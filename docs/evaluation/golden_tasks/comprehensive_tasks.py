...
def get_tasks_by_difficulty(difficulty: TaskDifficulty) -> List[GoldenTask]:
    ...
    return [
        ...
        if task.metadata.get("difficulty") == difficulty.value
    ]
...
def get_tasks_by_tier(tier: ExpectedTier) -> List[GoldenTask]:
    ...
    return [
        task
        for task in get_all_tasks()
        if task.metadata.get("expected_tier") == tier.value
    ]
...
def get_high_priority_tasks() -> List[GoldenTask]:
    ...
    return [
        ...
        if task.metadata.get("difficulty") in ["easy", "medium"]
    ]
...
def get_tasks_by_subdomain(subdomain: str) -> List[GoldenTask]:
    ...
    return [
        task for task in get_all_tasks() if task.metadata.get("subdomain") == subdomain
    ]
...
def get_tasks_by_confidence_threshold(min_confidence: float) -> List[GoldenTask]:
    ...
    return [
        task for task in get_all_tasks() if task.expected_confidence >= min_confidence
    ]
