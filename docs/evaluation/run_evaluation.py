...
class EvaluationRunner:
    ...
    async def run_evaluation(
        self, tenant_id: str, evaluation_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._store_evaluation_results(
                evaluation_id, tenant_id, results, metrics, report
            )
    ...
    async def _run_evaluation_tasks(
        self, tasks: List[Any], tenant_id: str, evaluation_id: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _run_task_batch(
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                logger.error(
                    "Task execution failed", task_id=tasks[i].task_id, error=str(result)
                )
    ...
    async def _run_single_task(
        self, task: Any, tenant_id: str, evaluation_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Single task execution failed", task_id=task.task_id, error=str(e)
            )
    ...
    async def _store_evaluation_results(
        ...
        try:
            ...
            await self.redis_client.setex(
                key, 86400, json.dumps(evaluation_data)
            )  # 24 hours TTL
            ...
            logger.error(
                "Failed to store evaluation results",
                evaluation_id=evaluation_id,
                error=str(e),
            )
...
async def main():
    ...
    parser.add_argument(
        "--max-tasks", type=int, default=50, help="Maximum number of tasks"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Batch size for parallel execution"
    )
    parser.add_argument(
        "--threshold-pass-rate", type=float, default=0.8, help="Pass rate threshold"
    )
    parser.add_argument(
        "--threshold-average-score",
        type=float,
        default=0.7,
        help="Average score threshold",
    )
    parser.add_argument("--categories", nargs="+", help="Task categories to evaluate")
    parser.add_argument(
        "--difficulties", nargs="+", help="Task difficulties to evaluate"
    )
    ...
    parser.add_argument(
        "--ci-mode", action="store_true", help="Run in CI mode with strict thresholds"
    )
