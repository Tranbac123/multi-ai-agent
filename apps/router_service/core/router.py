...
class RouterEngine:
    ...
    async def route(self, request: RouterDecisionRequest) -> RouterDecisionResponse:
        ...
        try:
            ...
            logger.error("Routing failed", error=str(e), request_id=str(request.request_id))
            raise
    ...
    def _should_early_exit(self, features: TextFeatures) -> bool:
        ...
        return (
            features.token_count < 50 and
            features.json_schema_complexity < 0.3 and
            features.novelty_score < 0.2 and
            features.historical_failure_rate < 0.1 and
            not any(features.domain_flags.values())
        )
    ...
    def _get_tier_cost(self, tier: RouterTier) -> float:
        ...
        costs = {
            RouterTier.SLM_A: 0.001,
            RouterTier.SLM_B: 0.005,
            RouterTier.LLM: 0.02
        }
    ...
    def _check_policy_escalation(self, features: TextFeatures, tier: RouterTier) -> bool:
        """Check if policy requires escalation."""
        # Escalate to LLM for high-risk domains
        high_risk_domains = ['finance', 'legal', 'medical']
        if any(features.domain_flags.get(domain, False) for domain in high_risk_domains):
    ...
    def _update_metrics(self, tier: RouterTier, decision_time: float):
        ...
        self.metrics["avg_decision_time_ms"] = (
            (current_avg * (total_requests - 1) + decision_time * 1000) / total_requests
        )
