...
class FeatureExtractor:
    ...
    async def extract_features(
        ...
        try:
            ...
            schema_strictness = (
                results[1] if not isinstance(results[1], Exception) else 0.5
            )
            ...
            historical_failure_rate = (
                results[4] if not isinstance(results[4], Exception) else 0.1
            )
            user_tier = (
                results[5] if not isinstance(results[5], Exception) else "standard"
            )
            request_complexity = (
                results[6] if not isinstance(results[6], Exception) else 0.5
            )
    ...
    async def _calculate_schema_strictness(self, request: Dict[str, Any]) -> float:
        ...
        try:
            ...
            has_structured_data = any(
                key in request for key in ["json", "schema", "format"]
            )

            # Check for validation rules
            has_validation = any(
                key in request for key in ["validation", "constraints", "rules"]
            )
    ...
    async def _extract_domain_flags(self, request: Dict[str, Any]) -> Dict[str, bool]:
        ...
        try:
            ...
            for domain, patterns in self.domain_patterns.items():
                domain_flags[domain] = any(
                    pattern in text_content for pattern in patterns
                )
    ...
    async def _calculate_novelty_score(
        self, request: Dict[str, Any], tenant_id: str
    ) -> float:
        ...
        try:
            ...
            recent_requests = await self.redis.lrange(
                recent_requests_key, 0, 99
            )  # Last 100 requests
    ...
    async def _cache_features(self, cache_key: str, features: RouterFeatures) -> None:
        ...
        try:
            ...
            await self.redis.setex(
                cache_key, self._cache_ttl, json.dumps(features_dict)
            )
        except Exception as e:
    ...
    async def batch_extract_features(
        self, requests: List[Tuple[Dict[str, Any], str, str]]
    ) -> List[RouterFeatures]:
            ...
            for i in range(0, len(requests), self._batch_size):
                ...
                for result in batch_results:
                    if isinstance(result, Exception):
                        logger.warning(
                            "Failed to extract features in batch", error=str(result)
                        )
