...
class RouterV2(EnhancedRouterBase):
    ...
    async def route_request(
        self, request: Dict[str, Any], tenant_id: str, user_id: str
    ) -> RouterDecision:
        ...
        try:
            ...
            (
                ...
                canary_tier,
                canary_info,
            ) = await self.canary_manager.should_use_canary(
            ...
            (
                ...
                confidence,
                should_escalate,
            ) = await self.classifier.classify(features, tenant_id)
            ...
            if escalation_decision.should_escalate:
                ...
                confidence = min(
                    confidence, 0.7
                )  # Reduce confidence for escalated requests

            decision_time = (time.time() - start_time) * 1000

            # Record metrics
            await self._record_router_metrics(
                tenant_id, final_tier, decision_time, features
            )
    ...
    async def record_outcome(
        ...
        try:
            ...
            is_canary, _, _ = await self.canary_manager.should_use_canary(
    ...
    async def _record_router_metrics(
        ...
        try:
            metrics_key = (
                f"router_metrics:{tenant_id}:{int(time.time() // 60)}"  # Per minute
            )
            ...
            await self.redis.hincrbyfloat(
                metrics_key, "total_decision_time", decision_time_ms
            )
            await self.redis.hincrby(metrics_key, "total_tokens", features.token_count)

            # Record feature metrics
            await self.redis.hincrbyfloat(
                metrics_key, "total_complexity", features.request_complexity
            )
            await self.redis.hincrbyfloat(
                metrics_key, "total_novelty", features.novelty_score
            )
            await self.redis.hincrbyfloat(
                metrics_key, "total_failure_rate", features.historical_failure_rate
            )
    ...
    async def get_router_statistics(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            escalation_stats = await self.early_exit.get_escalation_statistics(
                tenant_id
            )
        ...
        try:
            ...
            for metrics_key in metrics_keys:
                ...
                if metrics_data:
                    ...
                    for tier in Tier:
                        tier_distribution[tier.value] += int(
                            metrics_data.get(f"decisions_{tier.value}", 0)
                        )
                    total_decision_time += float(
                        metrics_data.get("total_decision_time", 0)
                    )
                    ...
                    total_failure_rate += float(
                        metrics_data.get("total_failure_rate", 0)
                    )
