...
class CalibratedClassifier:
    ...
    async def classify(
        ...
        try:
            ...
            calibrated_scores = self._apply_temperature_scaling(
                tier_scores, calibration.temperature
            )

            # Get best tier and confidence
            best_tier, confidence = self._get_best_tier_and_confidence(
                calibrated_scores
            )
            ...
            await self._update_calibration_data(
                tenant_id, features, best_tier, confidence
            )
        ...
        try:
            ...
            if calibration_data:
                return CalibrationData(
                    temperature=float(
                        calibration_data.get("temperature", self.default_temperature)
                    ),
                    confidence_threshold=float(
                    ...
                    last_updated=float(
                        calibration_data.get("last_updated", time.time())
                    ),
                )
    ...
    def _apply_temperature_scaling(
        self, scores: Dict[Tier, float], temperature: float
    ) -> Dict[Tier, float]:
    ...
    def _get_best_tier_and_confidence(
        self, scores: Dict[Tier, float]
    ) -> Tuple[Tier, float]:
    ...
    def _should_escalate(
        self, features: RouterFeatures, confidence: float, calibration: CalibrationData
    ) -> bool:
        ...
        try:
            ...
            if features.user_tier == "enterprise" and features.request_complexity > 0.7:
                return True
    ...
    async def calibrate_temperature(self, tenant_id: str) -> None:
        ...
        try:
            ...
            optimal_temperature = await self._calculate_optimal_temperature(
                decision_keys
            )
    ...
    async def _evaluate_temperature(
        self, temperature: float, decision_keys: List[bytes]
    ) -> float:
