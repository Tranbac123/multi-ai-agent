...
class MetricsCollector:
    ...
    async def record_decision(
        ...
        try:
            ...
            await self._record_cost_comparison(
                tenant_id, expected_cost, actual_cost, timestamp
            )

            # Record latency comparison if provided
            if expected_latency_ms is not None and actual_latency_ms is not None:
                await self._record_latency_comparison(
                    tenant_id, expected_latency_ms, actual_latency_ms, timestamp
                )
    ...
    async def _record_latency(
        self, tenant_id: str, latency_ms: float, timestamp: float
    ) -> None:
        """Record decision latency."""
        try:
    ...
    async def _record_tier_distribution(
        self, tenant_id: str, tier: Tier, timestamp: float
    ) -> None:
        """Record tier distribution."""
        try:
    ...
    async def _record_success_failure(
        self, tenant_id: str, success: bool, timestamp: float
    ) -> None:
        """Record success/failure."""
        try:
    ...
    async def _record_cost_comparison(
        self, tenant_id: str, expected_cost: float, actual_cost: float, timestamp: float
    ) -> None:
        """Record cost comparison."""
        try:
    ...
    async def _get_latency_metrics(self, tenant_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            latency_values = await self.redis.zrange(
    ...
    async def _record_latency_comparison(
        self,
        tenant_id: str,
