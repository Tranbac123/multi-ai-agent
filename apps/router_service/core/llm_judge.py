        ...
        try:
            ...
            for tier in ["A", "B", "C"]:
                ...
                if tier_key in performance_data:
                    ...
                    historical_performance[tier] = json.loads(
                        performance_data[tier_key]
                    )
                else:
...
"""
        ...
        try:
            ...
            if is_misroute:
                ...
                await self.redis_client.hincrby(
                    misroute_key, f"{predicted_tier}_to_{corrected_tier}", 1
                )
    ...
    async def get_judge_performance(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            judgments = await self.redis_client.lrange(
                judgments_key, 0, 99
            )  # Last 100 judgments
