...
class BanditPolicy:
    ...
    async def select_arm(
        ...
        try:
            ...
            selected_arm, selection_info = await self._select_arm_strategy(
                arm_values, tenant_id
            )

            # Update selection metrics
            await self._update_selection_metrics(
                tenant_id, selected_arm, selection_info
            )
            ...
            for tier in Tier:
                ...
                if arm_data and int(arm_data["pulls"]) > 0:
                    ...
                    value = (
                        expected_reward
                        - expected_cost
                        - (self.config.lambda_error * expected_error_rate)
                    )
    ...
    async def _select_arm_strategy(
        self, arm_values: Dict[Tier, float], tenant_id: str
        ...
        try:
            ...
            if total_pulls < self.config.min_pulls:
                ...
                if await self._should_explore():
                    ...
                    selected_arm, ucb_value = await self._select_ucb_arm(
                        tenant_id, arm_values
                    )
                    selection_info = {
    ...
    async def _select_ucb_arm(
        self, tenant_id: str, arm_values: Dict[Tier, float]
    ) -> Tuple[Tier, float]:
    ...
    async def _update_selection_metrics(
        self, tenant_id: str, selected_arm: Tier, selection_info: Dict[str, Any]
    ) -> None:
        """Update selection metrics."""
        try:
            ...
            await self.redis.hincrby(
                metrics_key, f'strategy_{selection_info["strategy"]}', 1
            )
