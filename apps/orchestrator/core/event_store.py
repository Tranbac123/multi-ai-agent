...
class EventStore:
    ...
    async def store_event(
        self, run_id: UUID, event_type: str, data: Dict[str, Any]
    ) -> None:
    ...
    async def get_events(self, run_id: UUID) -> List[Dict[str, Any]]:
        ...
        with tracer.start_as_current_span("get_events") as span:
            ...
            try:
                ...
                logger.debug(
                    "Events retrieved", run_id=str(run_id), event_count=len(events)
                )

                return events

            except Exception as e:
                logger.error("Failed to get events", run_id=str(run_id), error=str(e))
                raise
    ...
    async def get_latest_event(
        self, run_id: UUID, event_type: Optional[str] = None
    ...
    async def replay_events(self, run_id: UUID) -> AgentRun:
        ...
        with tracer.start_as_current_span("replay_events") as span:
            ...
            try:
                ...
                logger.error(
                    "Failed to replay events", run_id=str(run_id), error=str(e)
                )
                raise
    ...
    async def _reconstruct_run_state(self, events: List[Dict[str, Any]]) -> AgentRun:
        ...
        try:
            ...
            run = AgentRun(
                ...
                finished_at=datetime.fromisoformat(run_state["finished_at"])
                if run_state["finished_at"]
                else None,
    ...
    async def _apply_event(
        self, run_state: Dict[str, Any], event: Dict[str, Any]
    ) -> None:
    ...
    async def get_event_statistics(self, run_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to get event statistics", run_id=str(run_id), error=str(e)
            )
            raise
