...
class SagaOrchestrator:
    ...
    async def execute_saga(self, saga_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                ...
                logger.info(
                    "Saga completed successfully", saga_id=saga_id, name=saga.name
                )
            else:
            ...
            try:
                saga = self.active_sagas.get(
                    saga_id
                ) or await self._get_saga_definition(saga_id)
                if saga:
                    await self._execute_compensation(saga)
                    saga.status = SagaStatus.COMPENSATED
                    await self._update_saga_status(saga_id, SagaStatus.COMPENSATED)
                ...
                logger.error(
                    "Compensation failed", saga_id=saga_id, error=str(comp_error)
                )
    ...
    async def _execute_saga_sequential(self, saga: SagaDefinition) -> Dict[str, Any]:
        ...
        for step in saga.steps:
            try:
                ...
                await self._update_step_status(
                    saga.saga_id, step.step_id, StepStatus.RUNNING
                )

                # Execute step with timeout
                result = await asyncio.wait_for(
                    step.operation(), timeout=step.timeout_ms / 1000.0
                )
                ...
                await self._update_step_status(
                    saga.saga_id, step.step_id, StepStatus.COMPLETED
                )
                ...
                step.completed_at = time.time()
                await self._update_step_status(
                    saga.saga_id, step.step_id, StepStatus.FAILED
                )
        ...
        return {"success": True, "results": results, "saga_id": saga.saga_id}
    ...
    async def _execute_saga_parallel(self, saga: SagaDefinition) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Parallel saga execution failed", saga_id=saga.saga_id, error=str(e)
            )
            return {"success": False, "error": str(e), "saga_id": saga.saga_id}
    ...
    async def _execute_compensation(self, saga: SagaDefinition) -> None:
        ...
        try:
            ...
            logger.info(
                "Starting saga compensation", saga_id=saga.saga_id, name=saga.name
            )
            ...
            for step in completed_steps:
                if step.compensation:
                    try:
                        ...
                        await self._update_step_status(
                            saga.saga_id, step.step_id, StepStatus.COMPENSATED
                        )
            ...
            logger.info(
                "Saga compensation completed", saga_id=saga.saga_id, name=saga.name
            )
    ...
    async def _store_saga_definition(self, saga: SagaDefinition) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to store saga definition", saga_id=saga.saga_id, error=str(e)
            )
    ...
    async def _update_step_status(
        self, saga_id: str, step_id: str, status: StepStatus
    ) -> None:
