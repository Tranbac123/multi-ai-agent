...
class BaseTool:
    ...
    async def execute_with_resilience(self, func, *args, **kwargs):
        ...
        return await self.timeout_handler.execute(
            self.retry_policy.execute(self.circuit_breaker.execute(func)),
            *args,
            **kwargs,
        )
...
class CRMTool(BaseTool):
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(tool_call_id=tool_call.id, success=True, result=result)

        except Exception as e:
            logger.error(
                "CRM tool execution failed", error=str(e), tool_call_id=tool_call.id
            )
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            if action == "create_order":
                ...
                result = await self.get_orders_by_customer(
                    data.get("customer_id"), data.get("status")
                )
            ...
            logger.error(
                "Order tool execution failed", error=str(e), tool_call_id=tool_call.id
            )
            return ToolResult(tool_call_id=tool_call.id, success=False, error=str(e))
...
class PaymentTool(BaseTool):
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            logger.error(
                "Payment tool execution failed", error=str(e), tool_call_id=tool_call.id
            )
            return ToolResult(tool_call_id=tool_call.id, success=False, error=str(e))
...
class KnowledgeBaseTool(BaseTool):
        ...
        self.tool_spec = ToolSpec(
            ...
            inputs_schema={
                ...
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": ["search_faq", "get_faq", "search_products"],
                    },
                    "data": {"type": "object"},
                },
                "required": ["action", "data"],
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            logger.error(
                "Knowledge base tool execution failed",
                error=str(e),
