...
class EvalIntegration:
        ...
        try:
            ...
            if config.scope in [EvalScope.ALL, EvalScope.GOLDEN_TASKS]:
                try:
                    ...
                    performance_metrics.update(
                        self._extract_performance_metrics(golden_results)
                    )
            ...
            if config.scope in [EvalScope.ALL, EvalScope.EPISODE_REPLAY]:
                try:
                    ...
                    performance_metrics.update(
                        self._extract_replay_metrics(replay_results)
                    )
                except Exception as e:
            ...
            if config.scope in [EvalScope.ALL, EvalScope.INTEGRATION]:
                try:
                    ...
                    performance_metrics.update(
                        self._extract_integration_metrics(integration_results)
                    )
                except Exception as e:
            ...
            if config.scope in [EvalScope.ALL, EvalScope.PERFORMANCE]:
                try:
                    ...
                    performance_metrics.update(
                        self._extract_performance_benchmark_metrics(perf_results)
                    )
                except Exception as e:
                    error_msg = f"Performance benchmarks failed: {str(e)}"
                    errors.append(error_msg)
                    logger.error(
                        "Performance benchmarks evaluation failed", error=error_msg
                    )
    ...
    async def _run_golden_tasks(self) -> Dict[str, Any]:
        ...
        results["faq"] = {
            ...
            "avg_execution_time_ms": sum(r.execution_time_ms for r in faq_results)
            / len(faq_results),
        ...
        results["order"] = {
            ...
            "passed": len(
                [r for r in order_results if r.status == TaskStatus.COMPLETED]
            ),
            "failed": len([r for r in order_results if r.status == TaskStatus.FAILED]),
            "avg_execution_time_ms": sum(r.execution_time_ms for r in order_results)
            / len(order_results),
        ...
        results["lead"] = {
            ...
            "passed": len(
                [r for r in lead_results if r.status == TaskStatus.COMPLETED]
            ),
            "failed": len([r for r in lead_results if r.status == TaskStatus.FAILED]),
            "avg_execution_time_ms": sum(r.execution_time_ms for r in lead_results)
            / len(lead_results),
    ...
    async def _run_episode_replay(self) -> Dict[str, Any]:
        ...
        replay_id = await self.episode_replay.replay_episode(
            episode_id=episode_id, replay_mode=ReplayMode.EXACT
        )
    ...
    async def _run_performance_benchmarks(self) -> Dict[str, Any]:
        ...
        faq_avg_time = (
            sum(r.execution_time_ms for r in faq_results) / len(faq_results)
            if faq_results
            else 0
        )
        ...
        results["performance"] = {
            ...
            "throughput_tasks_per_second": len(faq_results) / faq_duration
            if faq_duration > 0
            else 0,
        }
    ...
    def _extract_performance_metrics(
        ...
        for task_type, results in golden_results.items():
            ...
            if "passed" in results and "total_tasks" in results:
                success_rate = (
                    results["passed"] / results["total_tasks"]
    ...
    def _extract_integration_metrics(
    ...
    def _extract_performance_benchmark_metrics(
        self, perf_results: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    async def _check_quality_gates(
        ...
        performance_thresholds = (
            config.performance_thresholds or self.default_performance_thresholds
        )
                ...
                if result_type == "golden_tasks":
                    for task_type, task_data in result_data.items():
                        if task_data.get("failed", 0) > 0:
                            logger.warning(
                                "Quality gate failed: tasks failed",
                                task_type=task_type,
            ...
            for metric_name, threshold in performance_thresholds.items():
                if metric_name in performance_metrics:
                    if performance_metrics[metric_name] > threshold:
                        logger.warning(
                            "Quality gate failed: performance threshold exceeded",
                            metric=metric_name,
        ...
        if quality_gates.get("no_critical_errors", True):
            if errors:
                logger.warning(
                    "Quality gate failed: critical errors present",
