...
1. **Check Service Health**
   ...
   kubectl get pods -l app=orchestrator -n production
   
   # Check recent logs for errors
   kubectl logs -l app=orchestrator -n production --tail=100 | grep ERROR
   ```

2. **Review Error Patterns**
   ```promql
   # Top error endpoints
   topk(10, sum by (endpoint) (rate(http_requests_total{service="orchestrator",status=~"5.."}[5m])))
   
   # Error breakdown by status code
   sum by (status) (rate(http_requests_total{service="orchestrator",status=~"5.."}[5m]))
   ```

3. **Check Dependencies**
   ```bash
   # Database connectivity
   kubectl exec -n production deployment/orchestrator -- ping database-host
...
1. **Identify Slow Endpoints**
   ...
   topk(10, histogram_quantile(0.95, sum by (endpoint) (rate(http_request_duration_seconds_bucket{service="orchestrator"}[5m]))))
   ```
...
1. **Check Pod Status**
   ```bash
   kubectl describe pod -l app=orchestrator -n production
   kubectl get events -n production --sort-by='.lastTimestamp' | grep orchestrator
   ```

2. **Review Startup Logs**
   ```bash
   kubectl logs -l app=orchestrator -n production --previous
   ```

3. **Check Resource Limits**
   ```bash
   kubectl top pod -l app=orchestrator -n production
   ```
...
1. **Memory Usage Breakdown**
   ```promql
   container_memory_usage_bytes{container="orchestrator"} / container_spec_memory_limit_bytes{container="orchestrator"} * 100
   ```

2. **Check for Memory Leaks**
   ```bash
   # Profile memory usage (if profiling enabled)
   curl http://orchestrator:6060/debug/pprof/heap
   ```
...
- **Request Rate**: `rate(http_requests_total{service="orchestrator"}[5m])`
- **Error Rate**: `rate(http_requests_total{service="orchestrator",status=~"5.."}[5m])`
- **Latency P95**: `histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{service="orchestrator"}[5m]))`
- **CPU Usage**: `rate(container_cpu_usage_seconds_total{container="orchestrator"}[5m]) * 100`
- **Memory Usage**: `container_memory_usage_bytes{container="orchestrator"}`

### Log Locations
- **Application Logs**: `kubectl logs -l app=orchestrator -n production`
- **Access Logs**: Available in Grafana Loki with label `{service="orchestrator"}`
- **Error Logs**: Filter by level=ERROR in log aggregation

### Trace Locations
- **Jaeger UI**: Search by service name `orchestrator`
...
- **Secondary**: Orchestrator Service Owner
...
kubectl get deployment orchestrator -n production -o jsonpath='{.spec.template.spec.containers[0].image}'

# Deploy new version (canary)
kubectl set image deployment/orchestrator orchestrator=new-image:tag -n production
...
curl -f http://orchestrator.production.svc.cluster.local:8002/healthz

# Verify metrics endpoint
curl http://orchestrator.production.svc.cluster.local:8002/metrics
```
