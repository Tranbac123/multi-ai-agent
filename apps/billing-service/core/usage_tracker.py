...
class UsageTracker:
    ...
    def set_pricing_rate(
        self, usage_type: UsageType, rate: float, unit: str = "per_unit"
    ) -> None:
        """Set pricing rate for usage type."""
        try:
            self.pricing_rates[usage_type.value] = {"rate": rate, "unit": unit}

            logger.info(
                "Pricing rate set", usage_type=usage_type.value, rate=rate, unit=unit
            )
    ...
    async def get_usage_summary(
        self, tenant_id: str, usage_type: UsageType, period: str = "monthly"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "usage_percentage": (total_usage / usage_limit.limit * 100)
                if usage_limit
                else 0,
                "cost": cost,
                "timestamp": time.time(),
    ...
    async def get_all_usage_summary(
        self, tenant_id: str, period: str = "monthly"
    ) -> Dict[str, Any]:
    ...
    async def check_usage_limit(
        self, tenant_id: str, usage_type: UsageType, additional_usage: float = 0
    ) -> Dict[str, Any]:
    ...
    async def _store_usage_record(self, usage_record: UsageRecord) -> None:
        ...
        try:
            ...
            record_data = {
                ...
                "metadata": str(usage_record.metadata)
                if usage_record.metadata
                else "{}",
            }
            ...
            usage_list_key = (
                f"usage_list:{usage_record.tenant_id}:{usage_record.usage_type.value}"
            )
    ...
    async def _check_usage_limits(self, tenant_id: str, usage_type: UsageType) -> None:
        ...
        try:
            ...
            if current_usage > usage_limit.limit:
                ...
                await self._trigger_limit_exceeded_alert(
                    tenant_id, usage_type, current_usage, usage_limit.limit
                )
    ...
    async def _get_usage_data(
        ...
        try:
            if period == "daily":
                ...
                return [
                    {"quantity": float(usage) if usage else 0, "timestamp": time.time()}
                ]
    ...
    async def _store_usage_limit(self, usage_limit: UsageLimit) -> None:
        ...
        try:
            limit_key = (
                f"usage_limit:{usage_limit.tenant_id}:{usage_limit.usage_type.value}"
            )
    ...
    async def _trigger_limit_exceeded_alert(
        self, tenant_id: str, usage_type: UsageType, current_usage: float, limit: float
    ) -> None:
        """Trigger limit exceeded alert."""
        try:
            # Store alert
