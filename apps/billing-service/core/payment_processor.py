...
class PaymentProcessor:
        ...
        try:
            ...
            for key in keys:
                try:
                    ...
                    logger.error("Failed to parse payment method", error=str(e), key=key)
    ...
    async def get_payment_history(self, tenant_id: str, limit: int = 100) -> List[Payment]:
                ...
                try:
                    ...
                    if payment_data:
                        payment = Payment(
                            ...
                            method=await self._get_payment_method(payment_data['method_id']),
                            ...
                            processed_at=float(payment_data['processed_at']) if payment_data.get('processed_at') else None,
    ...
    async def _handle_stripe_subscription_update(self, event_data: Dict[str, Any]) -> None:
    ...
    async def _handle_braintree_payment_success(self, event_data: Dict[str, Any]) -> None:
    ...
    async def _handle_braintree_payment_failure(self, event_data: Dict[str, Any]) -> None:
    ...
    async def _handle_braintree_subscription_canceled(self, event_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error("Failed to handle Braintree subscription canceled", error=str(e))
    
    async def _store_payment_method(self, payment_method: PaymentMethod) -> None:
        """Store payment method in Redis."""
        try:
            method_key = f"payment_method:{payment_method.tenant_id}:{payment_method.method_id}"
