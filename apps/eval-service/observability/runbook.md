...
- Alert: `Eval_ServiceHighErrorRate`

**Investigation Steps:**
1. **Check Service Health**
   ```bash
   # Check if service is up
   kubectl get pods -l app=eval-service -n production
   
   # Check recent logs for errors
   kubectl logs -l app=eval-service -n production --tail=100 | grep ERROR
   ```

2. **Review Error Patterns**
   ```promql
   # Top error endpoints
   topk(10, sum by (endpoint) (rate(http_requests_total{service="eval-service",status=~"5.."}[5m])))
   
   # Error breakdown by status code
   sum by (status) (rate(http_requests_total{service="eval-service",status=~"5.."}[5m]))
   ```

3. **Check Dependencies**
   ```bash
   # Database connectivity
   kubectl exec -n production deployment/eval-service -- ping database-host
...
- Alert: `Eval_ServiceHighLatency`

**Investigation Steps:**
1. **Identify Slow Endpoints**
   ```promql
   # Slowest endpoints (P95)
   topk(10, histogram_quantile(0.95, sum by (endpoint) (rate(http_request_duration_seconds_bucket{service="eval-service"}[5m]))))
   ```
...
- Alert: `Eval_ServiceServiceDown`

**Investigation Steps:**
1. **Check Pod Status**
   ```bash
   kubectl describe pod -l app=eval-service -n production
   kubectl get events -n production --sort-by='.lastTimestamp' | grep eval-service
   ```

2. **Review Startup Logs**
   ```bash
   kubectl logs -l app=eval-service -n production --previous
   ```

3. **Check Resource Limits**
   ```bash
   kubectl top pod -l app=eval-service -n production
   ```
...
- Potential OOM kills
- Alert: `Eval_ServiceHighMemoryUsage`

**Investigation Steps:**
1. **Memory Usage Breakdown**
   ```promql
   container_memory_usage_bytes{container="eval-service"} / container_spec_memory_limit_bytes{container="eval-service"} * 100
   ```

2. **Check for Memory Leaks**
   ```bash
   # Profile memory usage (if profiling enabled)
   curl http://eval-service:6060/debug/pprof/heap
   ```
...
- **Request Rate**: `rate(http_requests_total{service="eval-service"}[5m])`
- **Error Rate**: `rate(http_requests_total{service="eval-service",status=~"5.."}[5m])`
- **Latency P95**: `histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{service="eval-service"}[5m]))`
- **CPU Usage**: `rate(container_cpu_usage_seconds_total{container="eval-service"}[5m]) * 100`
- **Memory Usage**: `container_memory_usage_bytes{container="eval-service"}`

### Log Locations
- **Application Logs**: `kubectl logs -l app=eval-service -n production`
- **Access Logs**: Available in Grafana Loki with label `{service="eval-service"}`
- **Error Logs**: Filter by level=ERROR in log aggregation

### Trace Locations
- **Jaeger UI**: Search by service name `eval-service`
...
- **Secondary**: Eval Service Service Owner
...
kubectl get deployment eval-service -n production -o jsonpath='{.spec.template.spec.containers[0].image}'

# Deploy new version (canary)
kubectl set image deployment/eval-service eval-service=new-image:tag -n production

# Monitor deployment
kubectl rollout status deployment/eval-service -n production

# Rollback if needed
kubectl rollout undo deployment/eval-service -n production
```

### Health Check Verification
```bash
# Test health endpoint
curl -f http://eval-service.production.svc.cluster.local:8010/healthz

# Verify metrics endpoint
curl http://eval-service.production.svc.cluster.local:8010/metrics
```
