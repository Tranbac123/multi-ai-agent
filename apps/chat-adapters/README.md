...
- **📈 Grafana Dashboard**: `observability/dashboards/chat-adapters.json`
...
./platform/scripts/sync-observability.sh sync-service chat-adapters
...
./platform/scripts/sync-observability.sh --dry-run sync-service chat-adapters
```
...
```promql
# Request Rate
rate(http_requests_total{{service="chat-adapters"}}[5m])

# Error Rate
rate(http_requests_total{{service="chat-adapters",status=~"5.."}}[5m]) / rate(http_requests_total{{service="chat-adapters"}}[5m]) * 100

# Latency P95
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{{service="chat-adapters"}}[5m]))
