...
class DegradeRequest(BaseModel):
    ...
    duration_minutes: Optional[int] = Field(
        None, description="Duration in minutes, None for indefinite"
    )
    ...
    try:
        ...
        if request.duration_minutes:
            background_tasks.add_task(
                schedule_degrade_recovery, old_mode, request.duration_minutes * 60
            )

        logger.info(
            "Degrade mode changed",
...
async def create_alert(metric: str, value: float, threshold: float):
    ...
    existing_alert = next(
        (a for a in active_alerts if a.metric == metric and not a.resolved), None
    )
...
async def auto_trigger_degrade_mode(alert: CapacityAlert):
    ...
    if degrade_mode == DegradeMode.NORMAL:
        ...
        logger.warning(
            "Auto-triggered degrade mode", alert_id=alert.alert_id, metric=alert.metric
        )
...
async def schedule_degrade_recovery(original_mode: DegradeMode, delay_seconds: int):
    ...
    logger.info("Degrade mode recovered", original_mode=original_mode.value)
...
async def store_metrics(metrics: CapacityMetrics):
    ...
    await redis_client.setex(
        "current_metrics", 60, json.dumps(metrics.dict(), default=str)  # 1 minute TTL
    )

    # Store historical metrics
    timestamp = int(metrics.timestamp.timestamp())
    await redis_client.zadd(
        "metrics_history", {json.dumps(metrics.dict(), default=str): timestamp}
    )
