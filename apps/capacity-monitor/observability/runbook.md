# Capacity Monitor Service Runbook
...
1. **Check Service Health**
   ...
   kubectl get pods -l app=capacity-monitor -n production
   
   # Check recent logs for errors
   kubectl logs -l app=capacity-monitor -n production --tail=100 | grep ERROR
   ```

2. **Review Error Patterns**
   ```promql
   # Top error endpoints
   topk(10, sum by (endpoint) (rate(http_requests_total{service="capacity-monitor",status=~"5.."}[5m])))
   
   # Error breakdown by status code
   sum by (status) (rate(http_requests_total{service="capacity-monitor",status=~"5.."}[5m]))
   ```

3. **Check Dependencies**
   ```bash
   # Database connectivity
   kubectl exec -n production deployment/capacity-monitor -- ping database-host
...
1. **Identify Slow Endpoints**
   ...
   topk(10, histogram_quantile(0.95, sum by (endpoint) (rate(http_request_duration_seconds_bucket{service="capacity-monitor"}[5m]))))
   ```
...
1. **Check Pod Status**
   ```bash
   kubectl describe pod -l app=capacity-monitor -n production
   kubectl get events -n production --sort-by='.lastTimestamp' | grep capacity-monitor
   ```

2. **Review Startup Logs**
   ```bash
   kubectl logs -l app=capacity-monitor -n production --previous
   ```

3. **Check Resource Limits**
   ```bash
   kubectl top pod -l app=capacity-monitor -n production
   ```
...
1. **Memory Usage Breakdown**
   ```promql
   container_memory_usage_bytes{container="capacity-monitor"} / container_spec_memory_limit_bytes{container="capacity-monitor"} * 100
   ```

2. **Check for Memory Leaks**
   ```bash
   # Profile memory usage (if profiling enabled)
   curl http://capacity-monitor:6060/debug/pprof/heap
   ```
...
- **Request Rate**: `rate(http_requests_total{service="capacity-monitor"}[5m])`
- **Error Rate**: `rate(http_requests_total{service="capacity-monitor",status=~"5.."}[5m])`
- **Latency P95**: `histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{service="capacity-monitor"}[5m]))`
- **CPU Usage**: `rate(container_cpu_usage_seconds_total{container="capacity-monitor"}[5m]) * 100`
- **Memory Usage**: `container_memory_usage_bytes{container="capacity-monitor"}`

### Log Locations
- **Application Logs**: `kubectl logs -l app=capacity-monitor -n production`
- **Access Logs**: Available in Grafana Loki with label `{service="capacity-monitor"}`
- **Error Logs**: Filter by level=ERROR in log aggregation

### Trace Locations
- **Jaeger UI**: Search by service name `capacity-monitor`
...
- **Secondary**: Capacity Monitor Service Owner
...
kubectl get deployment capacity-monitor -n production -o jsonpath='{.spec.template.spec.containers[0].image}'

# Deploy new version (canary)
kubectl set image deployment/capacity-monitor capacity-monitor=new-image:tag -n production

# Monitor deployment
kubectl rollout status deployment/capacity-monitor -n production

# Rollback if needed
kubectl rollout undo deployment/capacity-monitor -n production
```

### Health Check Verification
```bash
# Test health endpoint
curl -f http://capacity-monitor.production.svc.cluster.local:8011/healthz

# Verify metrics endpoint
curl http://capacity-monitor.production.svc.cluster.local:8011/metrics
```
