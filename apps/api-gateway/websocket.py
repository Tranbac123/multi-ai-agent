...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        self.tenant_connections: Dict[str, Set[str]] = {}  # tenant_id -> set of session_ids
        self.user_sessions: Dict[str, str] = {}  # user_id -> session_id
        self.rate_limiter = RateLimiter()
    
    async def connect(self, websocket: WebSocket, session_id: str, tenant_id: str, user_id: Optional[str] = None):
    ...
    async def send_personal_message(self, message: dict, session_id: str, tenant_id: str):
        """Send message to specific session with tenant validation."""
        if session_id not in self.active_connections:
            logger.warning("Attempted to send message to non-existent session", 
                          session_id=session_id, tenant_id=tenant_id)
            return
        
        # Verify tenant access
        if tenant_id not in self.tenant_connections or session_id not in self.tenant_connections[tenant_id]:
    ...
    async def send_typing_indicator(self, session_id: str, tenant_id: str, is_typing: bool = True):
    ...
    async def broadcast_to_tenant(self, message: dict, tenant_id: str, exclude_session: Optional[str] = None):
...
class WebSocketHandler:
    ...
    async def handle_message(self, websocket: WebSocket, session_id: str, tenant_id: str, data: dict):
        """Handle incoming WebSocket message with enhanced processing."""
        try:
            # Check rate limits
            if not await manager.check_rate_limit(session_id, tenant_id):
                await self._send_error(websocket, session_id, tenant_id, 
            ...
            if message_type == "message":
                ...
                await self._handle_workflow_request(websocket, session_id, tenant_id, data)
            else:
                await self._send_error(websocket, session_id, tenant_id, 
            ...
            logger.error("Error handling WebSocket message", 
                        ...
                        tenant_id=tenant_id, 
                        error=str(e))
            await self._send_error(websocket, session_id, tenant_id, "Internal server error")
    
    async def _handle_chat_message(self, websocket: WebSocket, session_id: str, tenant_id: str, data: dict):
        ...
        try:
            ...
            response = await self._process_message_with_workflows(text, data.get("context", {}), tenant_id)
            ...
            logger.error("Error processing message", 
                        ...
                        tenant_id=tenant_id, 
                        error=str(e))
            await self._send_error(websocket, session_id, tenant_id, "Failed to process message")
        finally:
            # Stop typing indicator
            await manager.send_typing_indicator(session_id, tenant_id, False)
    
    async def _process_message_with_workflows(self, text: str, context: dict, tenant_id: str) -> dict:
        """Process message through YAML workflow system."""
    ...
    async def _handle_workflow_request(self, websocket: WebSocket, session_id: str, tenant_id: str, data: dict):
        ...
        try:
            ...
            result = await self._execute_workflow(workflow_name, workflow_data, tenant_id)
    ...
    async def _execute_workflow(self, workflow_name: str, workflow_data: dict, tenant_id: str) -> dict:
    ...
    async def _send_response(self, websocket: WebSocket, session_id: str, tenant_id: str, response: dict):
    ...
    async def _send_error(self, websocket: WebSocket, session_id: str, tenant_id: str, error_message: str):
    ...
    async def _handle_typing(self, websocket: WebSocket, session_id: str, tenant_id: str, data: dict):
        """Handle typing indicator."""
        is_typing = data.get("is_typing", False)
        await manager.send_typing_indicator(session_id, tenant_id, is_typing)
    
    async def _handle_ping(self, websocket: WebSocket, session_id: str, tenant_id: str, data: dict):
        """Handle ping message."""
        pong_message = {
            "type": "pong",
            "timestamp": datetime.utcnow().isoformat()
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await handler.handle_message(websocket, session_id, tenant_id, message_data)
            except json.JSONDecodeError:
                await handler._send_error(websocket, session_id, tenant_id, "Invalid JSON format")
