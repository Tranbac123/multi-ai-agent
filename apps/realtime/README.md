...
#### Python
```python
from realtime_client import ApiClient, Configuration
from realtime_client.api.default_api import DefaultApi

config = Configuration(host="http://localhost:8004")
...
#### TypeScript
```typescript
import { DefaultApi, Configuration } from '@company/realtime-client';
...
func main() {
    config := realtime.NewConfiguration()
    config.BasePath = "http://localhost:8004"
    client := realtime.NewAPIClient(config)
...
- **Dashboard**: [observability/dashboards/realtime.json](observability/dashboards/realtime.json)
...
- **📈 Grafana Dashboard**: `observability/dashboards/realtime.json`
...
./platform/scripts/sync-observability.sh sync-service realtime
...
./platform/scripts/sync-observability.sh --dry-run sync-service realtime
```
...
```promql
# Request Rate
rate(http_requests_total{{service="realtime"}}[5m])

# Error Rate
rate(http_requests_total{{service="realtime",status=~"5.."}}[5m]) / rate(http_requests_total{{service="realtime"}}[5m]) * 100

# Latency P95
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{{service="realtime"}}[5m]))
