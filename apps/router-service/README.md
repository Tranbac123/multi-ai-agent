...
- **📈 Grafana Dashboard**: `observability/dashboards/router-service.json`
...
./platform/scripts/sync-observability.sh sync-service router-service
...
./platform/scripts/sync-observability.sh --dry-run sync-service router-service
```
...
```promql
# Request Rate
rate(http_requests_total{{service="router-service"}}[5m])

# Error Rate
rate(http_requests_total{{service="router-service",status=~"5.."}}[5m]) / rate(http_requests_total{{service="router-service"}}[5m]) * 100

# Latency P95
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{{service="router-service"}}[5m]))
