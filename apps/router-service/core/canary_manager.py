...
class CanaryManager:
    ...
    async def should_use_canary(
        ...
        try:
            ...
            is_canary_user = await self._is_canary_user(tenant_id, user_id, canary_config.canary_percentage)
    ...
    async def record_canary_outcome(
        ...
        try:
            ...
            await self._update_canary_metrics(tenant_id, success, latency, quality_score)
    ...
    async def _get_canary_config(self, tenant_id: str) -> CanaryConfig:
        ...
        try:
            ...
            if config_data:
                return CanaryConfig(
                    tenant_id=tenant_id,
                    canary_percentage=float(config_data.get('canary_percentage', self.default_canary_percentage)),
                    quality_threshold=float(config_data.get('quality_threshold', self.default_quality_threshold)),
                    min_requests=int(config_data.get('min_requests', self.default_min_requests)),
                    evaluation_window=int(config_data.get('evaluation_window', self.default_evaluation_window)),
                    rollback_threshold=float(config_data.get('rollback_threshold', self.default_rollback_threshold))
                )
    ...
    async def _is_canary_user(
        ...
        try:
            ...
            if user_hash is None:
                ...
                await self.redis.setex(user_hash_key, 86400 * 30, user_hash)  # 30 days TTL
    ...
    async def _get_canary_tier(self, tenant_id: str, features: RouterFeatures) -> Optional[Tier]:
    ...
    async def _update_canary_metrics(
        ...
        try:
            ...
            if current_latency:
                ...
                total_requests = int(await self.redis.hget(metrics_key, 'total_requests'))
                new_latency = (current_latency * (total_requests - 1) + latency) / total_requests
            else:
                new_latency = latency
            ...
            if current_quality:
                ...
                total_requests = int(await self.redis.hget(metrics_key, 'total_requests'))
                new_quality = (current_quality * (total_requests - 1) + quality_score) / total_requests
            else:
    ...
    async def _initiate_rollback(self, tenant_id: str, reason: str) -> None:
        ...
        try:
            ...
            await self.redis.hset(rollback_key, mapping={
    ...
    async def get_canary_status(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                'rollback_status': rollback_data.get('status', CanaryStatus.INACTIVE.value) if rollback_data else CanaryStatus.INACTIVE.value,
                'rollback_reason': rollback_data.get('reason') if rollback_data else None
            }
