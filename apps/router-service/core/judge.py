...
class LLMJudge:
    ...
    async def judge(self, request: RouterDecisionRequest, features: TextFeatures) -> 'JudgeResult':
        ...
        try:
            ...
            logger.error("LLM judge failed", error=str(e), request_id=str(request.request_id))
            # Return fallback decision
            return self._fallback_decision(features)
    
    def _create_judge_prompt(self, request: RouterDecisionRequest, features: TextFeatures) -> str:
        """Create prompt for LLM judge."""
        prompt = f"""
...
"""
    ...
    def _parse_judge_response(self, response: str) -> 'JudgeResult':
        ...
        try:
            ...
            logger.error("Failed to parse judge response", error=str(e), response=response)
            raise
