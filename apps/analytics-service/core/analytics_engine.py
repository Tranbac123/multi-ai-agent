...
class AnalyticsEngine:
    ...
    def _initialize_warehouse_connection(self) -> None:
        ...
        try:
            if self.data_source == DataSource.CLICKHOUSE:
                ...
                self.warehouse_client = {
                    "type": "bigquery",
                    "project": self.warehouse_config.get("project_id"),
                }

            logger.info(
                "Warehouse connection initialized", data_source=self.data_source.value
            )
    ...
    async def get_comprehensive_analytics(
        self, tenant_id: str, time_window: str = "1h"
    ) -> TenantAnalytics:
        ...
        try:
            ...
            reliability_metrics = await self._get_reliability_metrics(
                tenant_id, time_window
            )
    ...
    async def _get_postgres_metrics(
        self, tenant_id: str, time_window: str
    ) -> KPIMetrics:
    ...
    async def _get_clickhouse_metrics(
        ...
        try:
            ...
            return self._get_enhanced_default_metrics(
                tenant_id, time_window, DataSource.CLICKHOUSE.value
            )
        except Exception as e:
    ...
    async def _get_bigquery_metrics(
        ...
        try:
            ...
            # For now, return enhanced default metrics
            return self._get_enhanced_default_metrics(
                tenant_id, time_window, DataSource.BIGQUERY.value
            )
        except Exception as e:
            logger.error("Failed to get BigQuery metrics", error=str(e))
            return self._get_default_metrics(tenant_id, time_window)

    def _get_enhanced_default_metrics(
        self, tenant_id: str, time_window: str, data_source: str
    ) -> KPIMetrics:
    ...
    async def _get_reliability_metrics(
        self, tenant_id: str, time_window: str
    ) -> Dict[str, Any]:
    ...
    def _get_default_tenant_analytics(
        self, tenant_id: str, time_window: str
    ) -> TenantAnalytics:
