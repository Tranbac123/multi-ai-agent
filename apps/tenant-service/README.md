...
#### Python
```python
from tenant_service_client import ApiClient, Configuration
from tenant_service_client.api.default_api import DefaultApi

config = Configuration(host="http://localhost:8008")
...
#### TypeScript
```typescript
import { DefaultApi, Configuration } from '@company/tenant-service-client';
...
func main() {
    config := tenant_service.NewConfiguration()
    config.BasePath = "http://localhost:8008"
    client := tenant_service.NewAPIClient(config)
...
- **Dashboard**: [observability/dashboards/tenant-service.json](observability/dashboards/tenant-service.json)
...
- **📈 Grafana Dashboard**: `observability/dashboards/tenant-service.json`
...
./platform/scripts/sync-observability.sh sync-service tenant-service
...
./platform/scripts/sync-observability.sh --dry-run sync-service tenant-service
```
...
```promql
# Request Rate
rate(http_requests_total{{service="tenant-service"}}[5m])

# Error Rate
rate(http_requests_total{{service="tenant-service",status=~"5.."}}[5m]) / rate(http_requests_total{{service="tenant-service"}}[5m]) * 100

# Latency P95
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{{service="tenant-service"}}[5m]))
