...
class EpisodeReplayEngine:
        ...
        try:
            ...
            for step in episode.steps:
                try:
                    ...
                    logger.error("Step replay failed", 
            ...
            replay_episode.total_duration_ms = sum(step.duration_ms for step in replay_episode.steps)
    ...
    async def _replay_step(
        ...
        try:
            ...
            if original_step.step_type == "input":
                replay_step.output_data = await self._replay_input_step(original_step, mode)
                
            elif original_step.step_type == "router_decision":
                replay_step.output_data = await self._replay_router_step(original_step, mode)
                
            elif original_step.step_type == "tool_call":
                replay_step.output_data = await self._replay_tool_step(original_step, mode)
                
            elif original_step.step_type == "response":
                replay_step.output_data = await self._replay_response_step(original_step, mode)
    ...
    async def _replay_router_step(
        ...
        if mode == ReplayMode.EXACT:
            ...
            return {
                "selected_tier": original_step.output_data.get("selected_tier", "SLM_A"),
                "confidence": original_step.output_data.get("confidence", 0.0),
    ...
    async def _adaptive_input_processing(self, original_step: EpisodeStep) -> Dict[str, Any]:
    ...
    async def _adaptive_router_decision(self, original_step: EpisodeStep) -> Dict[str, Any]:
    ...
    async def _adaptive_response_generation(self, original_step: EpisodeStep) -> Dict[str, Any]:
    ...
    async def _store_episode(self, episode: Episode) -> None:
        ...
        try:
            ...
            await self.redis.setex(episode_key, 86400, json.dumps(episode_data))  # 24 hours TTL
    ...
    async def get_episode_stats(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            durations = [e["total_duration_ms"] for e in episodes if e["total_duration_ms"]]
            ...
            logger.error("Failed to get episode stats", 
                       tenant_id=tenant_id,
                       error=str(e))
            return {}
    ...
    async def cleanup_old_episodes(self, days: int = 30) -> int:
        ...
        try:
            ...
            for key in keys:
                ...
                if episode_data:
                    data = json.loads(episode_data)
                    episode_time = datetime.fromisoformat(data["start_time"]).timestamp()
            ...
            logger.info("Episode cleanup completed", 
                       days=days,
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        episode_id = await engine.record_episode("tenant_123", "session_456", config, steps)
