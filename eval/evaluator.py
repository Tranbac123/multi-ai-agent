...
class Evaluator:
    ...
    async def evaluate_end_to_end(
        ...
        for task in tasks:
            try:
                ...
                logger.error(
                    "E2E evaluation failed for task",
                    task_id=task.task_id,
                    error=str(e)
                )
                results.append({
    ...
    def _filter_tasks(self, task_filter: Optional[Dict[str, Any]]) -> List[GoldenTask]:
        ...
        filtered_tasks = []
        for task in tasks:
            if task_filter.get("difficulty") and task.difficulty != task_filter["difficulty"]:
                continue
    ...
    async def _evaluate_router_task(self, router_engine, task: GoldenTask) -> Dict[str, Any]:
    ...
    async def _evaluate_agent_task(self, orchestrator_engine, task: GoldenTask) -> Dict[str, Any]:
    ...
    async def _evaluate_e2e_task(
        ...
        return {
            ...
            "overall_score": (router_result.get("judge_score", 0) + agent_result.get("judge_score", 0)) / 2
        }
    
    def _generate_router_report(self, results: List[Dict[str, Any]]) -> EvaluationReport:
        ...
        tier_accuracy = tier_correct / len(successful_results) if successful_results else 0.0
        ...
        cost_efficiency = 1.0 - (sum(cost_ratios) / len(cost_ratios) - 1.0) if cost_ratios else 0.0
        ...
        latency_performance = 1.0 - (sum(latency_ratios) / len(latency_ratios) - 1.0) if latency_ratios else 0.0
